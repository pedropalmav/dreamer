...
def main(argv=None):
    ...
    # TODO: change this block to a match block
    ...
    return Agent(
        ...
        elements.Config(
            **config.agent,
            ...
            batch_length=config.batch_length,
            replay_context=config.replay_context,
            replay_length=config.batch_length,
            replica=config.replica,
            replicas=config.replicas,
        ),
    )
...
def make_logger(config):
    ...
    for output in config.logger.outputs:
        # TODO: change this block to a match block and make own funciton
        if output == "jsonl":
...
def make_replay(config, folder, mode="train"):
    ...
    if config.replay.fracs.uniform < 1 and mode == "train":
        # TODO: make equivalent assert for pytorch
...
def make_env(config, index, **overrides):
    ...
    # TODO: Check if this is boilerplate
    if suite == "memmaze":
